---
sidebar: sidebar 
permalink: release-notes/dr-whats-new.html 
keywords: disaster recovery, bluexp, disaster recovery, vmware, vcenter, draas, disaster 
summary: NetApp Disaster Recovery é um serviço de recuperação de desastres baseado em nuvem que automatiza os fluxos de trabalho de recuperação de desastres. Com o NetApp Disaster Recovery, você pode replicar suas cargas de trabalho VMware locais que executam armazenamento ONTAP para um data center definido por software em nuvens públicas como um site de recuperação de desastres usando NetApp storage de nuvem. 
---
= Novidades no NetApp Disaster Recovery
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Descubra as novidades em NetApp Disaster Recovery.



[[16-september-2026]]
== 16 de setembro de 2026

Versão 4.3.7



[[kubernetes-support-in-netapp-disaster-recovery-is-now-generally-available-ga]]
=== O suporte do Kubernetes em NetApp Disaster Recovery agora está disponível para todos (GA)

O suporte para cargas de trabalho baseadas em Kubernetes no NetApp Disaster Recovery agora está disponível para o público em geral (GA). Com o suporte para cargas de trabalho baseadas em Kubernetes, você pode proteger aplicativos em execução em clusters Kubernetes, incluindo ambientes de OpenShift Virtualization. O NetApp Disaster Recovery é compatível com qualquer versão do Kubernetes atualmente suportada pelo Trident Protect, garantindo integração perfeita e recursos robustos de recuperação de desastres para aplicativos conteinerizados.

Para começar a proteger cargas de trabalho baseadas em Kubernetes, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/task-kubernetes-requirements.html["Requisitos do Kubernetes para NetApp Disaster Recovery"^].



[[resource-transformations]]
=== Transformações de recursos

A NetApp Disaster Recovery agora oferece suporte a transformações de recursos, permitindo que você modifique qualquer aspecto de uma carga de trabalho durante um failover. Esse recurso proporciona flexibilidade para adaptar cargas de trabalho a diferentes ambientes e configurações durante um evento de NetApp Disaster Recovery.

As transformações de recursos são aplicadas quando você cria um plano de replicação. Você pode criar, editar e excluir transformações de recursos na nova página Configurações. Para obter mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/task-resource-transformation.html["Gerenciar transformações de recursos"^].



[[script-execution]]
=== Execução de script

Agora você pode inserir scripts personalizados no fluxo de trabalho de failover para coordenar a inatividade e a retomada de aplicativos. Esse recurso ajuda a garantir que os aplicativos estejam devidamente preparados antes que um snapshot seja replicado e restaurado adequadamente, para que as operações possam ser retomadas sem problemas após o failover.

Para aprender como injetar um script personalizado, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"^].



[[new-workloads-page]]
=== Nova página de cargas de trabalho

O NetApp Disaster Recovery adicionou uma página de Workloads à interface do usuário, fornecendo uma visão abrangente de todos os workloads detectados e seu status de proteção. A página de Workloads permite que você monitore e gerencie seus workloads com mais eficiência.

Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/dashboard-view.html["Analisar o estado de saúde e proteção no NetApp Disaster Recovery"^].



[[settings-page]]
=== Página de configurações

O NetApp Disaster Recovery agora oferece uma página de Configurações para gerenciar centralmente os modelos de transformação de recursos e AppVaults. Esta página fornece uma interface única para configurar e gerenciar esses componentes críticos dos seus planos de NetApp Disaster Recovery. Para saber mais sobre as opções na página de Configurações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/task-resource-transformation.html["Gerenciar transformações de recursos"^] e link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/task-appvault.html["Gerenciar AppVaults"^].



[[general-improvements]]
=== Melhorias gerais

A funcionalidade de NetApp Disaster Recovery introduziu validações aprimoradas na página de criação do plano de replicação para garantir uma experiência de usuário mais tranquila e livre de erros.

O NetApp Disaster Recovery também aumentou a resiliência do serviço para tolerar timeouts de requisição intermitentes, melhorando o desempenho e a confiabilidade geral da interface do usuário.



[[31-august-2026]]
== 31 de agosto de 2026

Versão 4.3.6P1



[[improved-snapmirror-snapshot-management]]
=== Gerenciamento de snapshots aprimorado do SnapMirror

Agora você pode acionar o failover usando o snapshot mais recente, que é o método preferido quando você não está criando um novo snapshot.

Ao selecionar uma linha de base de snapshot específica, os snapshots de transferência do SnapMirror ficam ocultos por padrão. Isso ocorre porque eles podem ser excluídos durante um failover, o que impossibilita reproteger a carga de trabalho após o failover devido a um erro de "nenhum snapshot comum encontrado". Você ainda pode usar esses snapshots, mas precisará concluir uma confirmação adicional para reconhecer o risco.

Para obter mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html["Failover para um site remoto"^].



[[other-improvements]]
=== Outras melhorias

* Pequenas melhorias foram feitas para aumentar a resiliência do fluxo de trabalho de failover de teste.
* Um erro de validação ao adicionar um script foi corrigido.




[[10-august-2026]]
== 10 de agosto de 2026

Versão 4.3.6



[[improved-dashboard]]
=== Painel aprimorado

O painel de controle de NetApp Disaster Recovery foi atualizado com uma experiência aprimorada que oferece uma visão abrangente e imediata de todo o seu ambiente de NetApp Disaster Recovery. O novo painel inclui:

* *Visibilidade unificada das cargas de trabalho* — Monitore as cargas de trabalho do VMware e do Kubernetes juntamente com a integridade da infraestrutura subjacente a partir de um painel único.
* *Detecção proativa de problemas* — O painel agora destaca automaticamente os problemas ambientais, permitindo diagnósticos mais rápidos e reduzindo o tempo de resolução.
* *Utilização da capacidade* — métricas de capacidade agora são apresentadas diretamente no painel, facilitando a compreensão do consumo de recursos.


Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/dashboard-view.html["Veja a integridade dos seus planos de NetApp Disaster Recovery no ​​painel"^].



[[improved-reports]]
=== Relatórios aprimorados

As funcionalidades de geração de relatórios foram ampliadas para fornecer uma visão mais detalhada das atividades de Disaster Recovery em todo o seu ambiente:

* Gere relatórios com escopo definido para cargas de trabalho VMware, cargas de trabalho Kubernetes ou ambas, oferecendo a flexibilidade de personalizar os relatórios para seu público.
* Cada relatório fornece um resumo completo das atividades durante um período específico, garantindo total visibilidade dos eventos em seu ambiente de NetApp Disaster Recovery.


Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/reports.html["Crie relatórios de NetApp Disaster Recovery"^].



[[user-job-logging]]
=== Registro de tarefas do usuário

O NetApp Disaster Recovery aprimorou a responsabilidade e a auditabilidade das tarefas com o registro de tarefas em nível de usuário. Cada tarefa executada agora registra o usuário que a iniciou, fornecendo uma trilha de auditoria clara para fins de conformidade e solução de problemas. As tarefas acionadas automaticamente pelo NetApp Disaster Recovery são identificadas como "system", facilitando a distinção entre atividades iniciadas pelo usuário e atividades iniciadas pelo serviço.

Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/monitor-jobs.html["Monitorar trabalhos de NetApp Disaster Recovery"^].



[[performance-and-resource-optimization]]
=== Otimização de desempenho e recursos

Diversas melhorias internas foram implementadas para reduzir o consumo geral de recursos do sistema e aumentar o desempenho nas operações de NetApp Disaster Recovery.



[[13-july-2026]]
== 13 de julho de 2026

Versão 4.3.5



[[kubernetes-updates]]
=== Atualizações do Kubernetes

A Recuperação de Desastres introduziu as seguintes alterações para cargas de trabalho baseadas em Kubernetes:



==== Gerencie Trident Protect no menu Sites

O Disaster Recovery agora permite gerenciar o Trident Protect diretamente do menu Sites na interface do usuário do Disaster Recovery. Você pode atualizar o Trident Protect ou removê-lo do Disaster Recovery, simplificando o gerenciamento dos seus recursos do Kubernetes.

Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/manage.html["Gerenciar sites"^].



[[vmware-updates]]
=== Atualizações da VMware

A Recuperação de Desastres introduziu as seguintes alterações para cargas de trabalho baseadas em VMware:



==== Failover em datastores vazios

Os planos de replicação que incluem datastores vazios agora podem ser submetidos a failover com sucesso, sem a necessidade de excluí-los do plano ou forçar o failover. Durante o failover, o Disaster Recovery monta automaticamente o datastore vazio no site de recuperação de desastres, permitindo que as VMs sejam adicionadas e protegidas para operações de failback.

Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html["Failover para um local remoto"^].



==== vCenter automapeamento de rede

Agora você pode configurar relações de mapeamento de rede no nível do vCenter (ou seja, fora da caixa de diálogo do plano de replicação). Depois de configurados, o Disaster Recovery aplica automaticamente esses mapeamentos de rede quando você adiciona ou edita um plano de replicação, simplificando o gerenciamento do plano de replicação.

A Recuperação de Desastres continua a suportar substituições por plano. Os mapeamentos de rede existentes definidos em um plano de replicação ainda podem ser editados por rede; esses mapeamentos têm precedência sobre as definições de nível vCenter.



[[disaster-recovery-viewer-role-tightening]]
=== Reforço do papel do visualizador de disaster recovery

A funcionalidade de visualizar a aba "Upgrade System" foi removida da função de visualizador de Disaster Recovery. Essa alteração alinha a função com a experiência mais adequada para usuários com permissão somente de leitura. link:https://docs.netapp.com/us-en/console-setup-admin/reference-iam-disaster-rec-roles.html["Saiba mais sobre funções e permissões de usuário no NetApp Disaster Recovery"^].



[[system-performance-enhancements]]
=== Melhorias no desempenho do sistema

A Recuperação de Desastres introduziu mudanças para reduzir o consumo de recursos do sistema e melhorar o desempenho em todas as operações.



[[08-june-2026]]
== 08 de junho de 2026

Versão 4.3.4



[[kubernetes-updates]]
=== Atualizações do Kubernetes

A Recuperação de Desastres introduziu as seguintes alterações para cargas de trabalho baseadas em Kubernetes:



==== Teste o suporte a failover para cargas de trabalho do Kubernetes

O recurso de Recuperação de Desastres agora oferece suporte ao teste do processo de failover para cargas de trabalho do Kubernetes. Testar o failover permite validar os planos de recuperação de desastres. Ao concluir o teste, você pode limpar o failover de teste para que o sistema limpe automaticamente os recursos de teste, reduzindo a sobrecarga e garantindo a consistência do seu ambiente.

Para obter mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html#test-the-failover-process["Teste o processo de failover"^].



==== Falhas de teste programadas

Agora você pode automatizar os testes de recuperação de desastres agendando failovers de teste. Você pode definir agendamentos para failovers de teste recorrentes para validar continuamente os planos de recuperação sem intervenção manual, ajudando a manter a confiança nos objetivos de recuperação e a atender aos requisitos de conformidade.

Para obter mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/manage.html#manage-replication-plans["Gerenciar planos de replicação"^].



==== Descoberta agendada de cargas de trabalho do Kubernetes

Com a descoberta agendada, você pode controlar a frequência com que o Disaster Recovery verifica novas cargas de trabalho do Kubernetes. Você pode configurar os intervalos de descoberta para alinhar com sua cadência operacional, reduzindo a sobrecarga desnecessária do sistema enquanto garante que novas cargas de trabalho sejam detectadas e protegidas em tempo hábil.

Para obter mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/sites-add.html#edit-the-site-and-customize-the-discovery-schedule["Personalize o cronograma de descoberta"^].



[[vmware-updates]]
=== Atualizações da VMware



==== Proteção para datastores vazios

A Recuperação de Desastres agora permite configurar a proteção para armazenamentos de dados que não contêm máquinas virtuais (VMs). Quaisquer VMs adicionadas posteriormente ao armazenamento de dados são detectadas automaticamente pela Recuperação de Desastres, momento em que você precisa fornecer link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/sites-add.html#add-subnet-mapping-for-a-vcenter-site["informações de mapeamento"^].

Ao adicionar um datastore vazio a um plano de proteção, você deve remover o datastore do plano de proteção antes de executar um failover ou executar um failover forçado. Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html["Failover para um local remoto"^].



==== Restaurar armazenamentos de dados previamente excluídos para um plano de proteção

Os armazenamentos de dados que antes eram excluídos da proteção agora podem ser reincluídos. Essa melhoria proporciona maior flexibilidade no gerenciamento e na atualização das suas configurações de recuperação de desastres ao longo do tempo.

Para obter mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/manage.html["Gerenciar sites, grupos de recursos e planos de replicação"^].



==== Failover mais rápido para datastores baseados em VMFS

A Recuperação de Desastres aprimorou o desempenho de failover para datastores baseados em VMFS. As operações de reescaneamento do Host Bus Adapter (HBA) e montagem do datastore agora são executadas em paralelo, reduzindo os tempos gerais de failover e minimizando o tempo de inatividade durante eventos de recuperação.

Para obter mais informações sobre failover, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html["Fail over com recuperação de desastres"^].



[[11-may-2026]]
== 11 de maio de 2026

Versão 4.3.3



[[kubernetes-based-workload-support-preview]]
=== Suporte a cargas de trabalho baseadas em Kubernetes (prévia)

NetApp Disaster Recovery agora inclui suporte para cargas de trabalho baseadas em Kubernetes. Com o suporte ao Kubernetes, você pode proteger aplicativos em execução em clusters Kubernetes, incluindo ambientes de OpenShift Virtualization. Disaster Recovery oferece suporte a link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-requirements.html["qualquer versão do Kubernetes atualmente suportada pelo Trident Protect"^], garantindo integração perfeita e recursos robustos de recuperação de desastres para aplicativos conteinerizados.

Principais funcionalidades do suporte a cargas de trabalho baseadas em Kubernetes:

* **Ampla compatibilidade**: O Disaster Recovery é compatível com todas as versões do Kubernetes compatíveis com o Trident Protect, incluindo Red Hat OpenShift Virtualization.
* **Proteção centrada na aplicação**: Agora você pode proteger cargas de trabalho do Kubernetes no nível da aplicação, garantindo a recuperação de serviços críticos durante eventos de failover.
* **Orquestração simplificada**: O Disaster Recovery oferece fluxos de trabalho intuitivos para automatizar o backup e a replicação de cargas de trabalho do Kubernetes, aproveitando a mesma tecnologia eficiente SnapMirror usada para ambientes VMware.


O suporte para cargas de trabalho baseadas em Kubernetes está atualmente em versão prévia. Para começar a proteger cargas de trabalho baseadas em Kubernetes, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/task-kubernetes-requirements.html["Requisitos do Kubernetes para NetApp Disaster Recovery"^].



[[13-april-2026]]
== 13 de abril de 2026

Versão 4.3.2



[[google-cloud-vmware-engine-gcve-support-is-now-generally-available-ga]]
=== O suporte do Google Cloud VMware Engine (GCVE) agora está disponível para o público em geral (GA)

NetApp Disaster Recovery para GCVE agora está disponível para o público em geral (GA). Com o suporte GA, Disaster Recovery para GCVE agora oferece suporte a:

* *Snapshots gerenciados pela plataforma:* Gerencie backups e políticas de retenção de forma integrada por meio do ONTAP ou do Google Cloud Platform (GCP).
* *Vários datastores por plano de replicação:* Simplifique seus planos de NetApp Disaster Recovery consolidando vários datastores em uma única estratégia de replicação.
* *Forçar failover para sites de origem indisponíveis:* Alternar para um site de recuperação de desastres se a origem estiver indisponível.
* *Adie a resincronização do SnapMirror e restaure a replicação sob demanda:* Adie as resincronizações do SnapMirror para evitar impacto nas cargas de trabalho de produção durante períodos de pico de operação usando a opção "ignorar proteção" e, posteriormente, restaure a replicação sob demanda com a opção de reproteção.


Para obter mais informações sobre o suporte ao GCVE, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/release-notes/dr-limitations.html["Configurações suportadas para Disaster Recovery"^].



[[09-march-2026]]
== 09 de março de 2026

Versão 4.3.1



[[exclude-a-datastore-from-replication-plans]]
=== Excluir um datastore dos planos de replicação

Ao criar um plano de replicação no NetApp Disaster Recovery, agora você pode excluir datastores específicos da replicação. Para situações em que uma máquina virtual está configurada para usar vários datastores, isso permite limitar quais deles serão replicados.

Para obter detalhes, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"^].



[[removed-password-validation-from-vcenter-password]]
=== Validação de senha removida da senha do vCenter

A Recuperação de Desastres removeu as restrições de senha no vCenter para permitir senhas que contenham espaços.

Para obter mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/reference/evs-deploy-guide-add-vcenters.html["Adicionar clusters locais e do Amazon EVS vCenter no NetApp Disaster Recovery"^].



[[new-replication-plan-submenu]]
=== Novo submenu de plano de replicação

A Recuperação de Desastres agora inclui novas opções de menu para acionar atualizações do vCenter a partir do painel do plano de replicação. Você pode acessar o submenu no menu de ações do painel do plano de replicação: selecione **Integridade do Plano** e, em seguida, **Atualizar Origem vCenter / Atualizar Destino vCenter**.

Para obter mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html#modify-a-replication-plan["Modificar um plano de replicação"^].



[[improved-network-discovery]]
=== Descoberta de rede aprimorada

Disaster Recovery agora inclui descoberta de rede aprimorada para oferecer suporte a uma gama mais ampla de configurações de rede.



[[16-february-2026]]
== 16 de fevereiro de 2026

Versão 4.3.0



[[improved-user-interface]]
=== Interface de usuário aprimorada

A interface de usuário de Recuperação de Desastres foi reformulada para melhorar o desempenho e a experiência do usuário com base no feedback.



[[api-swagger-documentation]]
=== Documentação da API Swagger

A documentação Swagger da API de Recuperação de Desastres agora pode ser acessada em um navegador comum; o modo anônimo não é mais necessário. Para acessar a documentação da API, navegue até o painel de Recuperação de Desastres, selecione o menu de contexto no canto superior direito image:../media/use/icon-vertical-dots.png["Ícone de ação"] então **Documentação da API** para abrir a documentação Swagger. Você também pode link:https://api.bluexp.netapp.com/v1/services/draas/api/api-doc/draas["adicione o URL aos seus favoritos"^].



[[12-january-2026]]
== 12 de janeiro de 2026

Versão 4.2.9



[[support-for-multiple-console-agents-in-on-premises-environments]]
=== Suporte para vários agentes do Console em ambientes locais.

Se você utiliza a Recuperação de Desastres localmente, agora pode implantar um agente de console para cada instância do vCenter, melhorando a resiliência.

Por exemplo, se você tiver dois sites (Sites A e B), o Site A pode ter o agente de console A anexado ao vCenter 1, à implantação ONTAP 1 e à implantação ONTAP 2. O Site B pode ter o agente de console B anexado às implantações do vCenter 2 e do ONTAP 3 e 4.

Para obter informações sobre o agente do Console na Recuperação de Desastres, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-prerequisites.html#netapp-console-prerequisites["Crie o agente do Console"^].



[[add-vms-after-failover-for-replication-plans-using-datastore-based-protection]]
=== Adicionar VMs após failover para planos de replicação usando proteção baseada em armazenamento de dados

Quando o failover é acionado, qualquer plano de replicação que utilize proteção baseada em armazenamento de dados inclui as VMs que foram adicionadas ao armazenamento de dados, desde que tenham sido detectadas. Você deve fornecer os detalhes de mapeamento para as VMs adicionadas antes que o failover seja concluído.

Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html["Aplicativos de failover"^].



[[new-email-notifications]]
=== Novas notificações por e-mail

A Recuperação de Desastres agora fornece notificações por e-mail para os seguintes eventos:

* Aproximando-se do limite de utilização da capacidade
* Geração de relatório concluída
* Fracassos no trabalho
* Vencimento da licença ou violações




[[swagger-improvements]]
=== Melhorias no Swagger

Agora você pode acessar a documentação do Swagger diretamente do Disaster Recovery. No Disaster Recovery, selecione **Configurações** e depois **Documentação da API**. Você deve estar no modo privado ou anônimo para acessar link:https://api.bluexp.netapp.com/v1/services/draas/api/api-doc/draas["o link"^].



[[improved-user-interfaces]]
=== Interfaces de usuário aprimoradas

A Recuperação de Desastres agora oferece avisos aprimorados e soluções de erros. Esta versão corrige um erro que impedia a exibição de trabalhos cancelados na interface do usuário. Os trabalhos cancelados agora estão visíveis. Também há um novo aviso quando a mesma rede de destino é mapeada para várias redes de origem diferentes.



[[retain-vm-folder-structure-added-as-default-in-replication-plans]]
=== Manter a estrutura de pastas da VM adicionada como padrão nos planos de replicação

Ao criar um plano de replicação, a nova configuração padrão é manter a estrutura de pastas da máquina virtual. Se o destino da recuperação não tiver a hierarquia de pastas original, o Disaster Recovery a criará. Você pode desmarcar essa opção para ignorar a hierarquia de pastas original.

Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"^].



[[09-december-2025]]
== 09 de dezembro de 2025

Versão 4.2.8P1



[[folder-hierarchy-retention]]
=== Retenção da hierarquia de pastas

Por padrão, a Recuperação de Desastres mantém a hierarquia de inventário da VM (estrutura de pastas) em caso de failover.  Se o destino da recuperação não tiver a pasta necessária, a Recuperação de Desastres a criará.

Agora você pode substituir essa configuração designando uma nova pasta principal para a máquina virtual ou desmarcando a opção **Manter a hierarquia de pastas original**.

Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"].



[[streamlined-console-agent-updating]]
=== Atualização simplificada do agente do console

A Recuperação de Desastres agora oferece suporte a um processo simplificado para o uso de vários agentes do Console em um ambiente de trabalho.  Para alternar entre agentes do Console, você deve editar a configuração do vCenter, redescobrir as credenciais e atualizar os planos de replicação para usar o novo agente do Console.

Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/reference/switch-console-agent.html["Agentes do console de troca"].



[[01-december-2025]]
== 01 de dezembro de 2025

Versão 4.2.8



[[support-for-google-cloud-vmware-engine-using-google-cloud-netapp-volumes]]
=== Suporte para o Google Cloud VMware Engine usando o Google Cloud NetApp Volumes

O NetApp Disaster Recovery agora oferece suporte ao Google Cloud VMware Engine, utilizando o Google Cloud NetApp Volumes para operações de migração, failover, failback e teste.  Essa integração possibilita fluxos de trabalho de recuperação de desastres contínuos entre ambientes locais e o Google Cloud.

Certifique-se de revisar olink:https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-prerequisites.html["pré-requisitos"] elink:https://docs.netapp.com/us-en/data-services-disaster-recovery/release-notes/dr-limitations.html["limitações"] para o Google Cloud.



[[10-november-2025]]
== 10 de novembro de 2025

Versão 4.2.7



[[cascading-failover-support]]
=== Suporte a failover em cascata

Agora você pode configurar um relacionamento em cascata no ONTAP e usar qualquer um dos ramos desse relacionamento de replicação para recuperação de desastres.



[[downgrade-vmware-hardware-support-during-registration]]
=== Reduzir o suporte de hardware da VMware durante o registro

O Disaster Recovery agora oferece suporte ao downgrade do hardware VMware para uma versão anterior do vSphere durante o registro. Isso é útil quando o host ESX de origem está executando uma versão mais recente do que o site de recuperação de desastres.

Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html#select-vcenter-servers["Crie um plano de replicação no NetApp Disaster Recovery"].



[[graceful-shutdown]]
=== Encerramento elegante

A Recuperação de Desastres agora desliga as VMs de forma controlada, em vez de desligá-las completamente. Se uma determinada máquina virtual demorar mais de dez minutos para ser desligada, o sistema de Recuperação de Desastres a desliga automaticamente.



[[pre-backup-scripting-support]]
=== Suporte para scripts de pré-backup

Agora você pode inserir scripts personalizados no fluxo de trabalho de failover para serem executados antes da criação de um backup. A criação de scripts de pré-backup permite controlar o estado da máquina virtual antes da replicação de um snapshot e preparar a máquina virtual para uma transição. Por exemplo, você pode injetar um script que desmonta uma montagem NFS, a qual será remontada usando um script diferente após a recuperação de falha.

Para mais informações, consulte link:https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html#select-vcenter-servers["Crie um plano de replicação no NetApp Disaster Recovery"].



[[06-october-2025]]
== 06 de outubro de 2025

Versão 4.2.6



[[bluexp-disaster-recovery-is-now-netapp-disaster-recovery]]
=== A BlueXP disaster recovery agora é NetApp Disaster Recovery

A BlueXP disaster recovery foi renomeada para NetApp Disaster Recovery.



[[bluexp-is-now-netapp-console]]
=== BlueXP agora é NetApp Console

O NetApp Console, construído sobre a base aprimorada e reestruturada do BlueXP, oferece gerenciamento centralizado do armazenamento NetApp e dos NetApp Data Services em ambientes locais e em nuvem com nível empresarial, fornecendo insights em tempo real, fluxos de trabalho mais rápidos e administração simplificada, altamente segura e em conformidade.

Para obter detalhes sobre o que mudou, consulte o link:https://docs.netapp.com/us-en/console-relnotes/index.html["Notas de versão do NetApp Console"].



[[other-updates]]
=== Outras atualizações

* O suporte para o Amazon Elastic VMware Service (EVS) com o Amazon FSx for NetApp ONTAP estava em uma prévia pública. Com este lançamento, ele agora está disponível para o público em geral. Para mais detalhes, consultelink:../reference/evs-deploy-guide-introduction.html["Introdução ao NetApp Disaster Recovery usando o Amazon Elastic VMware Service e o Amazon FSx for NetApp ONTAP"] .
* Melhorias na descoberta de armazenamento, incluindo tempos de descoberta reduzidos para implantações locais
* Suporte ao Gerenciamento de Identidade e Acesso (IAM), incluindo controle de acesso baseado em função (RBAC) e permissões de usuário aprimoradas
* Suporte de visualização privada para solução Azure VMware e Cloud Volumes ONTAP. Com esse suporte, agora você pode configurar a proteção de recuperação de desastres do local para a solução Azure VMware usando o armazenamento Cloud Volumes ONTAP .




[[04-august-2025]]
== 04 de agosto de 2025

Versão 4.2.5P2



[[netapp-disaster-recovery-updates]]
=== Atualizações do NetApp Disaster Recovery

Esta versão inclui as seguintes atualizações:

* Melhorou o suporte do VMFS para lidar com o mesmo LUN apresentado por várias máquinas virtuais de armazenamento.
* Melhorou a limpeza de desmontagem do teste para lidar com o armazenamento de dados que já está sendo desmontado e/ou excluído.
* Mapeamento de sub-rede aprimorado para que agora valide se o gateway inserido está contido na rede fornecida.
* Foi corrigido um problema que poderia causar falha no plano de replicação se o nome da VM contivesse ".com".
* Foi removida uma restrição que impedia que o volume de destino fosse o mesmo que o volume de origem ao criar o volume como parte da criação do plano de replicação.
* Adicionou suporte para uma assinatura de pagamento conforme o uso (PAYGO) para o NetApp Intelligent Services no Azure Marketplace e adicionou um link para o Azure Marketplace na caixa de diálogo de teste gratuito.
+
Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-intro.html#licensing["Licenciamento de NetApp Disaster Recovery"] e https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-licensing.html["Configurar licenciamento para NetApp Disaster Recovery"] .





[[14-july-2025]]
== 14 de julho de 2025

Versão 4.2.5



[[user-roles-in-netapp-disaster-recovery]]
=== Funções de usuário no NetApp Disaster Recovery

O NetApp Disaster Recovery agora emprega funções para controlar o acesso que cada usuário tem a recursos e ações específicos.

O serviço usa as seguintes funções específicas do NetApp Disaster Recovery.

* *Administrador de recuperação de desastres*: execute quaisquer ações no NetApp Disaster Recovery.
* *Administrador de failover de recuperação de desastres*: execute ações de failover e migração no NetApp Disaster Recovery.
* *Administrador do aplicativo de recuperação de desastres*: Crie e modifique planos de replicação e inicie failovers de teste.
* *Visualizador de recuperação de desastres*: visualize informações no NetApp Disaster Recovery, mas não pode executar nenhuma ação.


Se estiver clicando no serviço NetApp Disaster Recovery e configurando-o pela primeira vez, você deverá ter a permissão *SnapCenterAdmin* ou ter a função *Organization Admin*.

Para mais detalhes, vejalink:https://docs.netapp.com/us-en/console-setup-admin/reference-iam-disaster-rec-roles.html["Funções e permissões do usuário no NetApp Disaster Recovery"] .

https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["Saiba mais sobre funções de acesso para todos os serviços"^].



[[other-updates-in-netapp-disaster-recovery]]
=== Outras atualizações no NetApp Disaster Recovery

* Descoberta de rede aprimorada
* Melhorias de escalabilidade:
+
** Filtragem dos metadados necessários em vez de todos os detalhes
** Melhorias na descoberta para recuperar e atualizar recursos de VM mais rapidamente
** Otimização de memória e otimização de desempenho para recuperação e atualização de dados
** Melhorias na criação de clientes do vCenter SDK e no gerenciamento de pools


* Gerenciamento de dados obsoletos na próxima descoberta agendada ou manual:
+
** Quando uma VM é excluída no vCenter, o NetApp Disaster Recovery agora a remove automaticamente do plano de replicação.
** Quando um armazenamento de dados ou rede é excluído no vCenter, o NetApp Disaster Recovery agora o exclui do plano de replicação e do grupo de recursos.
** Quando um cluster, host ou datacenter é excluído do vCenter, o NetApp Disaster Recovery agora o exclui do plano de replicação e do grupo de recursos.




* Em algumas situações, após uma operação de failback, o iGroup foi deixado para trás após a conclusão da operação. Esta atualização remove o iGroup se ele estiver obsoleto.
* Se o FQDN do NFS foi usado no plano de replicação, o NetApp Disaster Recovery agora o resolve para um endereço IP. Esta atualização é útil se o FQDN não puder ser resolvido no site de recuperação de desastres.
* Melhorias no alinhamento da interface do usuário
* Melhorias no log para capturar os detalhes de dimensionamento do vCenter após a descoberta bem-sucedida




[[30-june-2025]]
== 30 de junho de 2025

Versão 4.2.4P2



[[discovery-improvements]]
=== Melhorias na descoberta

Esta atualização melhora o processo de descoberta, o que reduz o tempo necessário para a descoberta.



[[23-june-2025]]
== 23 de junho de 2025

Versão 4.2.4P1



[[subnet-mapping-improvements]]
=== Melhorias no mapeamento de sub-redes

Esta atualização aprimora a caixa de diálogo Adicionar e editar mapeamento de sub-rede com uma nova funcionalidade de pesquisa.  Agora você pode encontrar rapidamente sub-redes específicas inserindo termos de pesquisa, facilitando o gerenciamento de mapeamentos de sub-redes.



[[09-june-2025]]
== 09 de junho de 2025

Versão 4.2.4



[[windows-local-administrator-password-solution-laps-support]]
=== Suporte à solução de senha de administrador local do Windows (LAPS)

O Windows Local Administrator Password Solution (Windows LAPS) é um recurso do Windows que gerencia e faz backup automaticamente da senha de uma conta de administrador local no Active Directory.

Agora você pode selecionar opções de mapeamento de sub-rede e verificar a opção LAPS fornecendo os detalhes do controlador de domínio.  Usando esta opção, você não precisa fornecer uma senha para cada uma de suas máquinas virtuais.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[13-may-2025]]
== 13 de maio de 2025

Versão 4.2.3



[[subnet-mapping]]
=== Mapeamento de sub-rede

Com esta versão, você pode gerenciar endereços IP em failover de uma nova maneira usando o mapeamento de sub-redes, que permite adicionar sub-redes para cada vCenter.  Ao fazer isso, você define o CIDR IPv4, o gateway padrão e o DNS para cada rede virtual.

Após o failover, o NetApp Disaster Recovery determina o endereço IP apropriado de cada vNIC observando o CIDR fornecido para a rede virtual mapeada e o usa para derivar o novo endereço IP.

Por exemplo:

* RedeA = 10.1.1.0/24
* RedeB = 192.168.1.0/24


A VM1 tem uma vNIC (10.1.1.50) que está conectada à RedeA.  A RedeA é mapeada para a RedeB nas configurações do plano de replicação.

Em caso de failover, o NetApp Disaster Recovery substitui a porção de rede do endereço IP original (10.1.1) e mantém o endereço do host (.50) do endereço IP original (10.1.1.50). Para a VM1, o NetApp Disaster Recovery analisa as configurações CIDR da NetworkB e usa a porção de rede da NetworkB, 192.168.1, mantendo a porção do host (.50) para criar o novo endereço IP para a VM1. O novo IP passa a ser 192.168.1.50.

Em resumo, o endereço do host permanece o mesmo, enquanto o endereço de rede é substituído pelo que estiver configurado no mapeamento de sub-rede do site.  Isso permite que você gerencie a reatribuição de endereços IP em caso de failover com mais facilidade, especialmente se você tiver centenas de redes e milhares de VMs para gerenciar.

Para obter detalhes sobre como incluir o mapeamento de sub-redes em seus sites, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/sites-add.html["Adicionar sites do servidor vCenter"] .



[[skip-protection]]
=== Proteção contra pulos

Agora você pode pular a proteção para que o serviço não crie automaticamente um relacionamento de proteção reversa após um failover do plano de replicação.  Isso é útil se você quiser executar operações adicionais no site restaurado antes de colocá-lo novamente online no NetApp Disaster Recovery.

Quando você inicia um failover, por padrão, o serviço cria automaticamente um relacionamento de proteção reversa para cada volume no plano de replicação, se o site de origem original estiver online.  Isso significa que o serviço cria um relacionamento SnapMirror do site de destino de volta ao site de origem.  O serviço também reverte automaticamente o relacionamento do SnapMirror quando você inicia um failback.

Ao iniciar um failover, agora você pode escolher a opção *Ignorar proteção*.  Com isso, o serviço não reverte automaticamente o relacionamento do SnapMirror .  Em vez disso, ele deixa o volume gravável em ambos os lados do plano de replicação.

Depois que o site de origem estiver online novamente, você poderá estabelecer a proteção reversa selecionando *Proteger recursos* no menu Ações do plano de replicação.  Isso tenta criar um relacionamento de replicação reversa para cada volume no plano.  Você pode executar esta tarefa repetidamente até que a proteção seja restaurada.  Quando a proteção for restaurada, você poderá iniciar um failback da maneira usual.

Para obter detalhes sobre ignorar a proteção, consulte  https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html["Falha na execução de aplicativos para um site remoto"].



[[snapmirror-schedule-updates-in-the-replication-plan]]
=== Atualizações de agendamento do SnapMirror no plano de replicação

O NetApp Disaster Recovery agora oferece suporte ao uso de soluções externas de gerenciamento de snapshots, como o agendador de políticas nativo ONTAP SnapMirror ou integrações de terceiros com o ONTAP.  Se cada armazenamento de dados (volume) no plano de replicação já tiver um relacionamento SnapMirror que esteja sendo gerenciado em outro lugar, você poderá usar esses instantâneos como pontos de recuperação no NetApp Disaster Recovery.

Para configurar, na seção Plano de replicação > Mapeamento de recursos, marque a caixa de seleção *Usar backups e agendamentos de retenção gerenciados pela plataforma* ao configurar o mapeamento de Datastores.

Quando a opção é selecionada, o NetApp Disaster Recovery não configura um agendamento de backup.  No entanto, você ainda precisa configurar um cronograma de retenção porque snapshots ainda podem ser tirados para operações de teste, failover e failback.

Depois que isso for configurado, o serviço não fará nenhum snapshot agendado regularmente, mas dependerá da entidade externa para tirar e atualizar esses snapshots.

Para obter detalhes sobre como usar soluções de snapshots externos no plano de replicação, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[16-april-2025]]
== 16 de abril de 2025

Versão 4.2.2



[[scheduled-discovery-for-vms]]
=== Descoberta agendada para VMs

O NetApp Disaster Recovery realiza a descoberta uma vez a cada 24 horas.  Com esta versão, agora você pode personalizar o cronograma de descoberta para atender às suas necessidades e reduzir o impacto no desempenho quando precisar.  Por exemplo, se você tiver um grande número de VMs, poderá definir o agendamento de descoberta para ser executado a cada 48 horas.  Se você tiver um pequeno número de VMs, poderá definir o agendamento de descoberta para ser executado a cada 12 horas.

Se não quiser agendar a descoberta, você pode desabilitar a opção de descoberta agendada e atualizar a descoberta manualmente a qualquer momento.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/sites-add.html["Adicionar sites do servidor vCenter"] .



[[resource-group-datastore-support]]
=== Suporte ao armazenamento de dados do grupo de recursos

Anteriormente, você só podia criar grupos de recursos por VMs.  Com esta versão, você pode criar um grupo de recursos por armazenamentos de dados.  Ao criar um plano de replicação e um grupo de recursos para esse plano, todas as VMs em um armazenamento de dados serão listadas.  Isso é útil se você tiver um grande número de VMs e quiser agrupá-las por armazenamento de dados.

Você pode criar um grupo de recursos com um armazenamento de dados das seguintes maneiras:

* Ao adicionar um grupo de recursos usando armazenamentos de dados, você pode ver uma lista de armazenamentos de dados.  Você pode selecionar um ou mais armazenamentos de dados para criar um grupo de recursos.
* Ao criar um plano de replicação e um grupo de recursos dentro do plano, você pode ver as VMs nos armazenamentos de dados.


Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[notifications-of-free-trial-or-license-expiration]]
=== Notificações de teste gratuito ou expiração de licença

Esta versão fornece notificações de que o teste gratuito irá expirar em 60 dias para garantir que você tenha tempo de obter uma licença.  Esta versão também fornece notificações sobre o dia em que a licença expira.



[[notification-of-service-updates]]
=== Notificação de atualizações de serviço

Com esta versão, um banner aparece na parte superior para indicar que os serviços estão sendo atualizados e que o serviço foi colocado em modo de manutenção.  O banner aparece quando o serviço está sendo atualizado e desaparece quando a atualização é concluída.  Embora você possa continuar trabalhando na interface do usuário enquanto a atualização estiver em andamento, não será possível enviar novos trabalhos.  Os trabalhos agendados serão executados após a conclusão da atualização e o serviço retornar ao modo de produção.



[[10-march-2025]]
== 10 de março de 2025

Versão 4.2.1



[[intelligent-proxy-support]]
=== Suporte a proxy inteligente

O agente do NetApp Console oferece suporte ao proxy inteligente.  O proxy inteligente é uma maneira leve, segura e eficiente de conectar seu sistema local ao NetApp Disaster Recovery.  Ele fornece uma conexão segura entre seu sistema e o NetApp Disaster Recovery sem exigir uma VPN ou acesso direto à Internet.  Essa implementação de proxy otimizada descarrega o tráfego de API dentro da rede local.

Quando um proxy é configurado, o NetApp Disaster Recovery tenta se comunicar diretamente com o VMware ou o ONTAP e usa o proxy configurado se a comunicação direta falhar.

A implementação do proxy de NetApp Disaster Recovery requer comunicação na porta 443 entre o agente do Console e quaisquer servidores vCenter e matrizes ONTAP usando um protocolo HTTPS.  O agente NetApp Disaster Recovery dentro do agente do Console se comunica diretamente com o VMware vSphere, o VC ou o ONTAP ao executar qualquer ação.

Para obter mais informações sobre o proxy inteligente para NetApp Disaster Recovery, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-setup.html["Configure sua infraestrutura para NetApp Disaster Recovery"] .

Para obter mais informações sobre a configuração geral de proxy no NetApp Console, consulte https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["Configurar o agente do Console para usar um servidor proxy"^] .



[[end-the-free-trial-any-time]]
=== Encerre o teste gratuito a qualquer momento

Você pode interromper o teste gratuito a qualquer momento ou esperar até que ele expire.

Ver https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-licensing.html#end-the-free-trial["Encerrar o teste gratuito"] .



[[19-february-2025]]
== 19 de fevereiro de 2025

Versão 4.2



[[asa-r2-support-for-vms-and-datastores-on-vmfs-storage]]
=== Suporte ASA r2 para VMs e datastores em armazenamento VMFS

Esta versão do NetApp Disaster Recovery oferece suporte para o ASA r2 para máquinas virtuais e datastores em armazenamento VMFS. Em um sistema ASA r2, o ONTAP oferece suporte à funcionalidade SAN essencial, removendo recursos não suportados em ambientes SAN.

Esta versão oferece suporte aos seguintes recursos para ASA r2:

* Provisionamento de grupo de consistência para armazenamento primário (somente grupo de consistência plano, ou seja, apenas um nível sem estrutura hierárquica)
* Operações de backup (grupo de consistência), incluindo automação SnapMirror


O suporte para ASA r2 no NetApp Disaster Recovery usa o ONTAP 9.16.1.

Embora os armazenamentos de dados possam ser montados em um volume ONTAP ou em uma unidade de armazenamento ASA r2, um grupo de recursos no NetApp Disaster Recovery não pode incluir um armazenamento de dados do ONTAP e um do ASA r2.  Você pode selecionar um armazenamento de dados do ONTAP ou um armazenamento de dados do ASA r2 em um grupo de recursos.



[[30-october-2024]]
== 30 de outubro de 2024



[[reporting]]
=== Relatórios

Agora você pode gerar e baixar relatórios para ajudar a analisar seu cenário.  Relatórios pré-projetados resumem failovers e failbacks, mostram detalhes de replicação em todos os sites e mostram detalhes do trabalho dos últimos sete dias.

Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/reports.html["Criar relatórios de recuperação de desastres"] .



[[30-day-free-trial]]
=== Teste gratuito de 30 dias

Agora você pode se inscrever para um teste gratuito de 30 dias do NetApp Disaster Recovery.  Anteriormente, os testes gratuitos eram de 90 dias.

Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-licensing.html["Configurar licenciamento"] .



[[disable-and-enable-replication-plans]]
=== Desabilitar e habilitar planos de replicação

Uma versão anterior incluía atualizações na estrutura de programação de testes de failover, o que era necessário para dar suporte a programações diárias e semanais.  Esta atualização exigiu que você desabilitasse e reabilitasse todos os planos de replicação existentes para que você pudesse usar os novos agendamentos de testes de failover diários e semanais.  Este é um requisito único.

Veja como:

. No menu, selecione *Planos de replicação*.
. Selecione um plano e selecione o ícone Ações para mostrar o menu suspenso.
. Selecione *Desativar*.
. Após alguns minutos, selecione *Ativar*.




[[folder-mapping]]
=== Mapeamento de pastas

Ao criar um plano de replicação e mapear recursos de computação, agora você pode mapear pastas para que as VMs sejam recuperadas em uma pasta especificada para datacenter, cluster e host.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[vm-details-available-for-failover-failback-and-test-failover]]
=== Detalhes da VM disponíveis para failover, failback e failover de teste

Quando ocorre uma falha e você está iniciando um failover, executando um failback ou testando o failover, agora você pode ver detalhes das VMs e identificar quais VMs não foram reiniciadas.

Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html["Falha na execução de aplicativos para um site remoto"] .



[[vm-boot-delay-with-ordered-boot-sequence]]
=== Atraso na inicialização da VM com sequência de inicialização ordenada

Ao criar um plano de replicação, agora você pode definir um atraso de inicialização para cada VM no plano.  Isso permite que você defina uma sequência para as VMs iniciarem, a fim de garantir que todas as suas VMs de prioridade um estejam em execução antes que as VMs de prioridade subsequentes sejam iniciadas.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[vm-operating-system-information]]
=== Informações do sistema operacional da VM

Ao criar um plano de replicação, agora você pode ver o sistema operacional de cada VM no plano.  Isso é útil para decidir como agrupar VMs em um grupo de recursos.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[vm-name-aliasing]]
=== Alias de nome de VM

Ao criar um plano de replicação, agora você pode adicionar um prefixo e um sufixo aos nomes das máquinas virtuais no site de recuperação de desastres. Isso permite que você use um nome mais descritivo para as máquinas virtuais no plano.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[clean-up-old-snapshots]]
=== Limpar instantâneos antigos

Você pode excluir quaisquer snapshots que não sejam mais necessários além da contagem de retenção especificada.  Os instantâneos podem se acumular ao longo do tempo quando você diminui sua contagem de retenção de instantâneos, e agora você pode removê-los para liberar espaço.  Você pode fazer isso a qualquer momento, sob demanda, ou ao excluir um plano de replicação.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/manage.html["Gerenciar sites, grupos de recursos, planos de replicação, armazenamentos de dados e informações de máquinas virtuais"] .



[[reconcile-snapshots]]
=== Reconciliar instantâneos

Agora você pode reconciliar snapshots que estão fora de sincronia entre a origem e o destino.  Isso pode ocorrer se os snapshots forem excluídos em um destino fora do NetApp Disaster Recovery.  O serviço exclui o snapshot na origem automaticamente a cada 24 horas.  No entanto, você pode fazer isso sob demanda.  Esse recurso permite que você garanta que os instantâneos sejam consistentes em todos os sites.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/manage.html["Gerenciar planos de replicação"] .



[[20-september-2024]]
== 20 de setembro de 2024



[[support-for-on-premises-to-on-premises-vmware-vmfs-datastores]]
=== Suporte para datastores VMware VMFS locais para locais

Esta versão inclui suporte para VMs montadas em datastores do sistema de arquivos de máquina virtual VMware vSphere (VMFS) para iSCSI e FC protegidos para armazenamento local.  Anteriormente, o serviço fornecia uma _prévia de tecnologia_ com suporte a armazenamentos de dados VMFS para iSCSI e FC.

Aqui estão algumas considerações adicionais sobre os protocolos iSCSI e FC:

* O suporte ao FC é para protocolos de front-end do cliente, não para replicação.
* O NetApp Disaster Recovery suporta apenas um único LUN por volume ONTAP .  O volume não deve ter vários LUNs.
* Para qualquer plano de replicação, o volume ONTAP de destino deve usar os mesmos protocolos que o volume ONTAP de origem que hospeda as VMs protegidas.  Por exemplo, se a origem usa um protocolo FC, o destino também deve usar FC.




[[02-august-2024]]
== 02 de agosto de 2024



[[support-for-on-premises-to-on-premises-vmware-vmfs-datastores-for-fc]]
=== Suporte para datastores VMware VMFS locais para locais para FC

Esta versão inclui uma _prévia tecnológica_ de suporte para VMs montadas em armazenamentos de dados do sistema de arquivos de máquina virtual (VMFS) VMware vSphere para FC protegido para armazenamento local.  Anteriormente, o serviço fornecia uma prévia de tecnologia com suporte a armazenamentos de dados VMFS para iSCSI.


NOTE: A NetApp não cobra por nenhuma capacidade de carga de trabalho visualizada.



[[job-cancel]]
=== Cancelamento de trabalho

Com esta versão, agora você pode cancelar um trabalho na interface do Job Monitor.

Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/monitor-jobs.html["Monitorar trabalhos"] .



[[17-july-2024]]
== 17 de julho de 2024



[[failover-test-schedules]]
=== Cronogramas de testes de failover

Esta versão inclui atualizações na estrutura de agendamento de testes de failover, que eram necessárias para dar suporte a agendamentos diários e semanais.  Esta atualização exige que você desabilite e reabilite todos os planos de replicação existentes para que você possa usar os novos agendamentos de testes de failover diários e semanais.  Este é um requisito único.

Veja como:

. No menu, selecione *Planos de replicação*.
. Selecione um plano e selecione o ícone Ações para mostrar o menu suspenso.
. Selecione *Desativar*.
. Após alguns minutos, selecione *Ativar*.




[[replication-plan-updates]]
=== Atualizações do plano de replicação

Esta versão inclui atualizações nos dados do plano de replicação, o que resolve o problema de "instantâneo não encontrado".  Isso requer que você altere a contagem de retenção em todos os planos de replicação para 1 e inicie um instantâneo sob demanda.  Este processo cria um novo backup e remove todos os backups mais antigos.

Veja como:

. No menu, selecione *Planos de replicação*.
. Selecione o plano de replicação, selecione a guia *Mapeamento de failover* e selecione o ícone de lápis *Editar*.
. Selecione a seta *Datastores* para expandi-la.
. Observe o valor da contagem de retenção no plano de replicação. Você precisa restaurar esse valor original quando terminar essas etapas.
. Reduza a contagem para 1.
. Inicie um snapshot sob demanda. Para isso, na página do plano de replicação, selecione o plano, selecione o ícone Ações e selecione *Tirar instantâneo agora*.
. Após a conclusão bem-sucedida do trabalho de instantâneo, aumente a contagem no plano de replicação de volta ao valor original anotado na primeira etapa.
. Repita essas etapas para todos os planos de replicação existentes.




[[05-july-2024]]
== 05 de julho de 2024

Esta versão do NetApp Disaster Recovery inclui as seguintes atualizações:



[[support-for-aff-a-series]]
=== Suporte para AFF série A

Esta versão oferece suporte às plataformas de hardware NetApp AFF série A.



[[support-for-on-premises-to-on-premises-vmware-vmfs-datastores]]
=== Suporte para datastores VMware VMFS locais para locais

Esta versão inclui uma _prévia tecnológica_ de suporte para VMs montadas em armazenamentos de dados do sistema de arquivos de máquina virtual (VMFS) VMware vSphere protegidos no armazenamento local.  Com esta versão, a recuperação de desastres é suportada em uma prévia de tecnologia para cargas de trabalho VMware locais para ambientes VMware locais com armazenamentos de dados VMFS.


NOTE: A NetApp não cobra por nenhuma capacidade de carga de trabalho visualizada.



[[replication-plan-updates]]
=== Atualizações do plano de replicação

Você pode adicionar um plano de replicação mais facilmente filtrando VMs por armazenamento de dados na página Aplicativos e selecionando mais detalhes de destino na página Mapeamento de recursos. Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[edit-replication-plans]]
=== Editar planos de replicação

Com esta versão, a página de mapeamentos de failover foi aprimorada para maior clareza.

Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/manage.html["Gerenciar planos"] .



[[edit-vms]]
=== Editar VMs

Com esta versão, o processo de edição de VMs no plano incluiu algumas pequenas melhorias na interface do usuário.

Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/manage.html["Gerenciar VMs"] .



[[fail-over-updates]]
=== Atualizações de failover

Antes de iniciar um failover, agora você pode determinar o status das VMs e se elas estão ligadas ou desligadas.  O processo de failover agora permite que você tire um snapshot ou escolha os snapshots.

Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html["Falha na execução de aplicativos para um site remoto"] .



[[failover-test-schedules]]
=== Cronogramas de testes de failover

Agora você pode editar os testes de failover e definir agendamentos diários, semanais e mensais para o teste de failover.

Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/manage.html["Gerenciar planos"] .



[[updates-to-prerequisite-information]]
=== Atualizações nas informações de pré-requisitos

As informações sobre pré-requisitos do NetApp Disaster Recovery foram atualizadas.

Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-prerequisites.html["Pré-requisitos do NetApp Disaster Recovery"] .



[[15-may-2024]]
== 15 de maio de 2024

Esta versão do NetApp Disaster Recovery inclui as seguintes atualizações:



[[replicating-vmware-workloads-from-on-premises-to-on-premises]]
=== Replicando cargas de trabalho do VMware de local para local

Agora isso foi lançado como um recurso de disponibilidade geral.  Anteriormente, era uma prévia de tecnologia com funcionalidade limitada.



[[licensing-updates]]
=== Atualizações de licenciamento

Com o NetApp Disaster Recovery, você pode se inscrever para um teste gratuito de 90 dias, comprar uma assinatura pré-paga (PAYGO) no Amazon Marketplace ou Bring Your Own License (BYOL), que é um arquivo de licença NetApp (NLF) que você obtém do seu representante de vendas NetApp ou do site de suporte NetApp (NSS).

Para obter detalhes sobre a configuração do licenciamento para o NetApp Disaster Recovery, consultelink:../get-started/dr-licensing.html["Configurar licenciamento"] .

https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-intro.html["Saiba mais sobre a NetApp Disaster Recovery"].



[[05-march-2024]]
== 05 de março de 2024

Esta é a versão de disponibilidade geral do NetApp Disaster Recovery, que inclui as seguintes atualizações.



[[licensing-updates]]
=== Atualizações de licenciamento

Com o NetApp Disaster Recovery, você pode se inscrever para um teste gratuito de 90 dias ou Traga sua própria licença (BYOL), que é um arquivo de licença NetApp (NLF) que você obtém do seu representante de vendas NetApp .  Você pode usar o número de série da licença para ativar o BYOL nas assinaturas do NetApp Console .  As cobranças do NetApp Disaster Recovery são baseadas na capacidade provisionada dos datastores.

Para obter detalhes sobre como configurar o licenciamento do NetApp Disaster Recovery, consulte [link para a documentação]. https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-licensing.html["Configurar licenciamento"] .

Para obter detalhes sobre o gerenciamento de licenças para *todos* os serviços de dados do NetApp Console , consulte https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-data-services-licenses.html["Gerenciar licenças para todos os serviços de dados do NetApp Console"^] .



[[edit-schedules]]
=== Editar agendamentos

Com esta versão, agora você pode configurar agendamentos para testar testes de conformidade e failover para garantir que eles funcionarão corretamente caso você precise deles.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie o plano de replicação"] .



[[01-february-2024]]
== 01 de fevereiro de 2024

Esta versão de pré-lançamento do NetApp Disaster Recovery inclui as seguintes atualizações:



[[network-enhancement]]
=== Aprimoramento de rede

Com esta versão, agora você pode redimensionar os valores de CPU e RAM da VM.  Agora você também pode selecionar um DHCP de rede ou um endereço IP estático para a VM.

* DHCP: se você escolher esta opção, fornecerá credenciais para a VM.
* IP estático: você pode selecionar as mesmas informações ou informações diferentes da VM de origem.  Se você escolher o mesmo que a fonte, não precisará inserir credenciais.  Por outro lado, se você optar por usar informações diferentes da fonte, poderá fornecer as credenciais, endereço IP, máscara de sub-rede, DNS e informações de gateway.


Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[custom-scripts]]
=== Scripts personalizados

Agora podem ser incluídos como processos pós-failover.  Com scripts personalizados, você pode fazer com que o NetApp Disaster Recovery execute seu script após um processo de failover.  Por exemplo, você pode usar um script personalizado para retomar todas as transações do banco de dados após a conclusão do failover.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/failover.html["Failover para um site remoto"] .



[[snapmirror-relationship]]
=== Relacionamento SnapMirror

Agora você pode criar um relacionamento SnapMirror enquanto desenvolve o plano de replicação.  Anteriormente, você tinha que criar o relacionamento fora do NetApp Disaster Recovery.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[consistency-groups]]
=== Grupos de consistência

Ao criar um plano de replicação, você pode incluir VMs de volumes diferentes e SVMs diferentes.  O NetApp Disaster Recovery cria um instantâneo de grupo de consistência incluindo todos os volumes e atualiza todos os locais secundários.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[vm-power-on-delay-option]]
=== Opção de atraso na inicialização da VM

Ao criar um plano de replicação, você pode adicionar VMs a um Grupo de Recursos.  Com Grupos de Recursos, você pode definir um atraso em cada VM para que elas sejam inicializadas em uma sequência atrasada.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[application-consistent-snapshot-copies]]
=== Cópias de instantâneo consistentes com o aplicativo

Você pode especificar a criação de cópias de Snapshot consistentes com o aplicativo.  O serviço desativará o aplicativo e, em seguida, fará um Snapshot para obter um estado consistente do aplicativo.

Para mais detalhes, consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[11-january-2024]]
== 11 de janeiro de 2024

Esta versão de pré-visualização do NetApp Disaster Recovery inclui as seguintes atualizações:



[[dashboard-more-quickly]]
=== Painel mais rápido

Com esta versão, você pode acessar informações em outras páginas do Painel mais rapidamente.

https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-intro.html["Saiba mais sobre a NetApp Disaster Recovery"].



[[20-october-2023]]
== 20 de outubro de 2023

Esta versão de pré-visualização do NetApp Disaster Recovery inclui as seguintes atualizações.



[[protect-on-premises-nfs-based-vmware-workloads]]
=== Proteja cargas de trabalho VMware locais baseadas em NFS

Agora, com o NetApp Disaster Recovery, você pode proteger suas cargas de trabalho VMware locais baseadas em NFS contra desastres em outro ambiente VMware local baseado em NFS, além da nuvem pública.  O NetApp Disaster Recovery orquestra a conclusão dos planos de recuperação de desastres.


NOTE: Com esta oferta de pré-visualização, a NetApp reserva-se o direito de modificar os detalhes, o conteúdo e o cronograma da oferta antes da disponibilidade geral.

https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-intro.html["Saiba mais sobre a NetApp Disaster Recovery"].



[[27-september-2023]]
== 27 de setembro de 2023

Esta versão de pré-visualização do NetApp Disaster Recovery inclui as seguintes atualizações:



[[dashboard-updates]]
=== Atualizações do painel

Agora você pode selecionar as opções no Painel de Controle, facilitando a revisão rápida das informações. Além disso, o Painel agora mostra o status de failovers e migrações.

Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/dashboard-view.html["Veja a integridade dos seus planos de recuperação de desastres no Painel"] .



[[replication-plan-updates]]
=== Atualizações do plano de replicação

* *RPO*: Agora você pode inserir o Objetivo de Ponto de Recuperação (RPO) e a contagem de Retenção na seção Datastores do plano de Replicação. Isso indica a quantidade de dados que deve existir e que não seja mais antiga que o tempo definido. Se, por exemplo, você definir 5 minutos, o sistema poderá perder até 5 minutos de dados se ocorrer um desastre, sem afetar as necessidades críticas dos negócios.
+
Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .

* *Melhorias de rede*: Ao mapear a rede entre os locais de origem e destino na seção de máquinas virtuais do plano de replicação, o NetApp Disaster Recovery agora oferece duas opções: DHCP ou IP estático. Anteriormente, apenas DHCP era suportado. Para IPs estáticos, você configura a sub-rede, o gateway e os servidores DNS. Além disso, agora você pode inserir credenciais para máquinas virtuais.
+
Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .

* *Editar agendamentos*: Agora você pode atualizar agendamentos de planos de replicação.
+
Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/manage.html["Gerenciar recursos"] .

* * Automação do SnapMirror *: Ao criar o plano de replicação nesta versão, você pode definir o relacionamento do SnapMirror entre os volumes de origem e de destino em uma das seguintes configurações:
+
** 1 a 1
** 1 para muitos em uma arquitetura fanout
** Muitos para 1 como um grupo de consistência
** Muitos para muitos
+
Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .







[[01-august-2023]]
== 01 de agosto de 2023



[[netapp-disaster-recovery-preview]]
=== Prévia do NetApp Disaster Recovery

O NetApp Disaster Recovery Preview é um serviço de recuperação de desastres baseado em nuvem que automatiza fluxos de trabalho de recuperação de desastres.  Inicialmente, com a visualização do NetApp Disaster Recovery , você pode proteger suas cargas de trabalho VMware locais baseadas em NFS executando armazenamento NetApp no ​​VMware Cloud (VMC) na AWS com o Amazon FSx para ONTAP.


NOTE: Com esta oferta de pré-visualização, a NetApp reserva-se o direito de modificar os detalhes, o conteúdo e o cronograma da oferta antes da disponibilidade geral.

https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-intro.html["Saiba mais sobre a NetApp Disaster Recovery"].

Esta versão inclui as seguintes atualizações:



[[resource-groups-update-for-boot-order]]
=== Atualização de grupos de recursos para ordem de inicialização

Ao criar um plano de recuperação de desastres ou replicação, você pode adicionar máquinas virtuais a grupos de recursos funcionais. Grupos de recursos permitem que você coloque um conjunto de máquinas virtuais dependentes em grupos lógicos que atendem aos seus requisitos. Por exemplo, grupos podem conter ordens de inicialização que podem ser executadas na recuperação. Com esta versão, cada grupo de recursos pode incluir uma ou mais máquinas virtuais. As máquinas virtuais serão ligadas com base na sequência em que você as incluir no plano. Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html#select-applications-to-replicate-and-assign-resource-groups["Selecione aplicativos para replicar e atribuir grupos de recursos"] .



[[replication-verification]]
=== Verificação de replicação

Depois de criar o plano de recuperação de desastres ou replicação, identificar a recorrência no assistente e iniciar uma replicação para um site de recuperação de desastres, a cada 30 minutos o NetApp Disaster Recovery verifica se a replicação está realmente ocorrendo de acordo com o plano. Você pode monitorar o progresso na página Job Monitor. Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/replicate.html["Replicar aplicativos para outro site"] .



[[replication-plan-shows-recovery-point-objective-rpo-transfer-schedules]]
=== O plano de replicação mostra os cronogramas de transferência do objetivo do ponto de recuperação (RPO)

Ao criar um plano de recuperação de desastres ou replicação, você seleciona as VMs. Nesta versão, agora você pode visualizar o SnapMirror associado a cada um dos volumes associados ao armazenamento de dados ou à VM. Você também pode ver os cronogramas de transferência de RPO associados ao cronograma do SnapMirror . O RPO ajuda você a determinar se seu cronograma de backup é suficiente para recuperação após um desastre. Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/drplan-create.html["Crie um plano de replicação"] .



[[job-monitor-update]]
=== Atualização do Job Monitor

A página Job Monitor agora inclui uma opção Atualizar para que você possa obter um status atualizado das operações. Consulte https://docs.netapp.com/us-en/data-services-disaster-recovery/use/monitor-jobs.html["Monitorar trabalhos de recuperação de desastres"] .



[[18-may-2023]]
== 18 de maio de 2023

Esta é a versão inicial do NetApp Disaster Recovery.



[[cloud-based-disaster-recovery-service]]
=== Serviço de recuperação de desastres baseado em nuvem

O NetApp Disaster Recovery é um serviço de recuperação de desastres baseado em nuvem que automatiza fluxos de trabalho de recuperação de desastres.  Inicialmente, com a visualização do NetApp Disaster Recovery , você pode proteger suas cargas de trabalho VMware locais baseadas em NFS executando armazenamento NetApp no ​​VMware Cloud (VMC) na AWS com o Amazon FSx para ONTAP.

link:https://docs.netapp.com/us-en/data-services-disaster-recovery/get-started/dr-intro.html["Saiba mais sobre a NetApp Disaster Recovery"].
