...
[role="lead"]
Você pode dar uma olhada rápida em todos os seus recursos de recuperação de desastres da NetApp ou analisar cada um deles em detalhes:
...
Embora você possa adicionar um grupo de recursos como parte da criação de um plano de replicação, pode ser mais conveniente adicionar os grupos separadamente e usá-los posteriormente no plano.  Você cria grupos de recursos por VMs ou por armazenamentos de dados.
...
Você pode executar as seguintes tarefas com grupos de recursos:
...
. Para executar ações com o grupo de recursos, selecione a opção *Ações*image:../use/icon-horizontal-dots.png["Ícone do menu Ações no serviço NetApp Disaster Recovery"] à direita e selecione qualquer uma das opções, como *Editar grupo de recursos* ou *Excluir grupo de recursos*.
...
Você pode reconciliar snapshots que estão fora de sincronia entre a origem e o destino.  Isso pode ocorrer se os snapshots forem excluídos em um destino fora do NetApp Disaster Recovery.  O serviço exclui automaticamente o snapshot na origem a cada 24 horas.  No entanto, você pode fazer isso sob demanda.  Esse recurso permite que você garanta que os instantâneos sejam consistentes em todos os sites.
...
. Na lista de planos de replicação, selecione a opção *Ações*image:../use/icon-horizontal-dots.png["Ícone do menu Ações no serviço NetApp Disaster Recovery"] à direita do plano e selecione *Reconciliar instantâneos*.
...
Você pode alterar quantos armazenamentos de dados são retidos.
