...
[role="lead"]
Após adicionar recursos e sites, crie um plano de replicação para proteger seus dados. O plano define os sites de origem e destino, os grupos de recursos, os mapeamentos de recuperação e o comportamento de inicialização dos seus aplicativos. Você também pode configurar agendamentos de conformidade e teste, além de realizar testes de failover sem afetar os dados de produção.
...
Você pode proteger várias VMs em vários datastores. NetApp Disaster Recovery cria grupos de consistência ONTAP para todos os volumes ONTAP que hospedam datastores de VMs protegidas.
...
** Se houver snapshots ausentes no site de destino, o serviço pode excluir os snapshots correspondentes no site de origem durante o próximo processo de reconciliação de snapshots agendado, dependendo da política padrão do SnapMirror para a relação.
...
=== Considerações sobre backups gerenciados pela plataforma

Se você planeja usar backups gerenciados pela plataforma (ou seja, snapshots gerenciados pelo ONTAP), é recomendável que você use a política de replicação *MirrorAll*. Consulte a tabela para entender o impacto das diferentes políticas de replicação do SnapMirror e por que *MirrorAll* é a opção recomendada.

[cols="10,10,80"]
|===


| Política | Recomendação | Notas 


| *MirrorAll* | Recomendado | Não utilize snapshots de transferência do SnapMirror para operações de failover ou failback. 


| *MirrorVault* .2+| Recomendação alternativa .2+| Certifique-se de que os snapshots selecionados durante o failover ou failback estejam presentes tanto nos volumes de origem quanto nos volumes de destino. Caso contrário, ocorrerá o erro: "Falha na transição para o estado 'snapmirrored'. Motivo: Nenhum snapshot comum encontrado." Não utilize snapshots de transferência do SnapMirror para operações de failover ou failback. 


| *Assíncrono* 


| *MirrorLatest* | _NÃO_ recomendado | Essa política resulta em o destino ter efetivamente apenas um snapshot comum para failover, o que é subótimo para cenários de recuperação de desastres. 
|===


[[considerations-for-disaster-recovery-managed-backups]]
=== Considerações sobre backups gerenciados pelo NetApp Disaster Recovery

Ao usar backups gerenciados do NetApp Disaster Recovery, as seguintes diretrizes se aplicam a relações do SnapMirror criadas fora do NetApp Disaster Recovery:

* As relações do SnapMirror podem suportar as políticas de replicação MirrorAll, MirrorVault, MirrorLatest e assíncrona.
* Não agende atualizações para esses relacionamentos, pois eles serão gerenciados pelo serviço de NetApp Disaster Recovery.
...
O assistente guia você por estas etapas:
...
* Não hospede recursos protegidos e não protegidos no mesmo datastore. Se recursos protegidos e não protegidos forem hospedados no mesmo datastore, você pode encontrar estes problemas:
+
...
+
** Para remover uma máquina virtual ou um datastore, passe o cursor sobre o nome do recurso e selecione *X*.
** Para organizar os recursos em vários grupos de recursos, selecione *Clique para adicionar outro grupo de recursos*. Depois de adicionar o grupo de recursos, você pode arrastar e soltar recursos entre os grupos. Selecione o ícone de lápis para editar o nome do grupo de recursos.
+
...
+
Ao definir a frequência de snapshot, você deve escolher uma frequência de pelo menos a cada cinco minutos.
...
+
Para o Azure, escolha a *Assinatura*, o *Grupo de recursos* e a *Conta de armazenamento* para o provedor de nuvem.

+
Para AWS, ONTAP S3 e StorageGRID, selecione a *Acount*, as *Credentials* e o *Bucket*.
...
.. Selecione **Adicionar**.
+
image:kubernetes-mapping.png["Captura de tela do mapeamento de classes de armazenamento para Kubernetes"]
...
==== Transformações de recursos e execuções de scripts

Para planos de replicação do Kubernetes, você pode aplicar transformações de recursos e execuções de scripts. Essas etapas são opcionais.

.Adicionar uma transformação de recurso
Adicionar uma transformação de recursos permite que você modifique qualquer aspecto de uma carga de trabalho durante um failover. As transformações de recursos oferecem flexibilidade para adaptar cargas de trabalho a diferentes ambientes e configurações durante um evento de recuperação de desastres. link:task-resource-transformation.html["Aprenda como criar e transformar recursos."^]

As transformações de recursos são aplicadas na ordem em que você as lista.

. No menu suspenso, selecione a transformação de recurso que você deseja adicionar.
. Escolha os recursos aos quais você deseja aplicar a transformação e, em seguida, adicione os valores necessários
. Revise a ordem das transformações para garantir que as operações sejam executadas na ordem apropriada.


.Adicionar uma execução de script
O NetApp Disaster Recovery pode executar um script antes ou depois de tirar um snapshot para gerenciar a quiescência e a retomada de um aplicativo. Executar essa ação garante que os aplicativos estejam devidamente preparados antes que um snapshot seja replicado e restaurado, e possam retomar as operações sem interrupções após o failover.

. Selecione *+ Adicionar script*.
. Selecione *Carregar* para escolher o arquivo de script.
. Escolha a *Etapa de execução*: *Pré-snapshot* para executar antes de criar o snapshot ou *Pós-snapshot* para executar depois.
. Selecione *Salvar*.
...
A seção Recursos de computação define onde as VMs são restauradas após o failover. Mapeie o data center e o cluster de origem do vCenter para um data center e um cluster de destino.

Opcionalmente, as VMs podem ser reiniciadas em um host ESXi específico do vCenter. Se o VMware DRS estiver habilitado, você pode mover a VM para um host alternativo automaticamente, se necessário, para atender à política de DR configurada.

Opcionalmente, você pode colocar todas as VMs neste plano de replicação em uma pasta exclusiva dentro do inventário do vCenter. Isso oferece uma maneira fácil de organizar rapidamente as VMs em failover dentro do vCenter.
...
Na guia Mapeamentos de failover, selecione a seta para baixo ao lado de *Redes virtuais*. Selecione a rede virtual de origem e a rede virtual de destino.
...
NOTE: Ative as *Configurações avançadas* para configurar *Scripts* e *Consistência do aplicativo*.
...
* *Tipo de endereço IP*: reconfigure a configuração da máquina virtual para corresponder aos requisitos da rede virtual de destino. O NetApp Disaster Recovery oferece duas opções: DHCP ou IP estático. Para IPs estáticos, configure a máscara de sub-rede, o gateway e os servidores DNS. Além disso, insira as credenciais para as máquinas virtuais.
+
...
+
*VMs executando Microsoft Windows*: podem executar scripts em lote (.bat) ou PowerShell (ps1). Os scripts do Windows podem usar argumentos de linha de comando. Formate cada argumento no formato  `arg_name$value`, onde  `arg_name` é o nome do argumento e  `$value` é o valor do argumento, e um ponto e vírgula separa cada par  `argument$value`.
...
+
** Inicialização sequencial: atribua a cada máquina virtual um número único para inicializá-las na ordem atribuída, por exemplo, 1,2,3,4,5.
...
Os dados gerenciados pelo servidor de banco de dados podem estar no mesmo datastore que a máquina virtual que hospeda o servidor de banco de dados ou em um datastore diferente. A tabela a seguir mostra as configurações compatíveis para snapshots consistentes com o aplicativo no NetApp Disaster Recovery:
...
* Para máquinas virtuais Windows, o serviço utiliza o Microsoft Volume Shadow Copy Service (VSS) para coordenar com qualquer tipo de servidor de banco de dados.
...
TIP: Analise o <<Considerações sobre backups gerenciados pela plataforma>> antes de fazer essas seleções.

* *Use backups gerenciados pela plataforma e cronogramas de retenção*: se você estiver usando uma solução externa de gerenciamento de snapshots, marque esta caixa. O Disaster Recovery oferece suporte ao uso de soluções externas de gerenciamento de snapshots, como o agendador de políticas nativo do ONTAP SnapMirror ou integrações de terceiro. Se cada datastore (volume) no plano de replicação já tiver uma relação do SnapMirror que está sendo gerenciada em outro local, você pode usar esses snapshots como pontos de recuperação no NetApp Disaster Recovery.
+
Quando esta opção é selecionada, o NetApp Disaster Recovery não configura um agendamento de backup. No entanto, você ainda precisa configurar um cronograma de retenção, pois snapshots podem ser criados para testes, failover e failback.
...
* *Datastores de origem e destino*: Se existirem vários relacionamentos SnapMirror (fan-out), você pode selecionar o destino a ser usado. Se um volume já tiver um relacionamento SnapMirror estabelecido, os datastores de origem e destino correspondentes serão exibidos. Se um volume não tiver um relacionamento SnapMirror, você pode criar um agora selecionando um cluster de destino, selecionando uma SVM de destino e fornecendo um nome de volume. O serviço criará o volume e o relacionamento SnapMirror.
+
...
** Durante o failover, você pode selecionar qualquer snapshot. Se você selecionar o snapshot mais recente, o serviço cria um backup sob demanda, atualiza o destino e usa esse snapshot para o failover.
