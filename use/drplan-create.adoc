...
. No menu NetApp Disaster Recovery, selecione *Planos de replicação* e selecione *Adicionar*. Se esta for a primeira vez que você está criando um plano de replicação, no Dashboard, selecione *Adicionar plano de replicação*.
. Insira um nome para o plano de replicação.
. Em *Origem vCenter*, selecione o vCenter onde os dados existem no menu suspenso. Em *Destino vCenter*, selecione o vCenter para o qual os dados serão replicados no menu suspenso.
+
...
* Certifique-se de que haja pelo menos uma máquina virtual (VM) no armazenamento de dados. Se não houver VMs no armazenamento de dados, o armazenamento de dados não será detectado.
* Um único armazenamento de dados não deve hospedar VMs protegidas por mais de um plano de replicação.
* Não hospede VMs protegidas e não protegidas no mesmo datastore. Se VMs protegidas e não protegidas forem hospedadas no mesmo datastore, você poderá encontrar estes problemas:
+
** A capacidade utilizada desse volume pode contribuir para o cálculo do licenciamento porque o Disaster Recovery usa SnapMirror, o que significa que o sistema replica volumes ONTAP inteiros. Nesse caso, o espaço do volume consumido tanto por VMs protegidas quanto por VMs não protegidas seria incluído nesse cálculo.
...
TIP: Crie um conjunto separado e dedicado de mapeamentos para seus testes de failover, a fim de evitar que as VMs sejam conectadas a redes de produção usando os mesmos endereços IP.
...
* *CPU* e *Memória (GB)*: nas configurações da máquina virtual, você pode opcionalmente redimensionar a CPU e a memória da VM.
+
