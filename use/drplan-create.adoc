...
[role="lead"]
Depois de adicionar recursos e sites, você estará pronto para criar um . Os planos de replicação gerenciam a proteção de dados. A criação da replicação envolve designar os sites de origem e destino, selecionar os grupos de recursos e escolher como os aplicativos devem ser restaurados e ligados. Por exemplo, você pode agrupar máquinas virtuais (VMs) associadas a um aplicativo ou pode agrupar aplicativos que tenham níveis semelhantes.
