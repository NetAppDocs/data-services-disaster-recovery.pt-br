...
[role="lead"]
Depois de adicionar sites de recursos, você pode criar grupos de recursos para agrupar VMs, datastores ou clusters Kubernetes como uma única unidade. Os grupos de recursos permitem que você organize um conjunto de recursos dependentes em grupos lógicos que atendam aos seus requisitos. Por exemplo, você pode agrupar VMs associadas a um aplicativo ou pode agrupar aplicativos que tenham níveis semelhantes. Como outro exemplo, os grupos podem conter ordens de inicialização atrasada que podem ser executadas durante a recuperação.
...
* Ao criar um plano de recuperação de desastres ou de replicação. Se você tiver muitas VMs hospedadas em um cluster de origem vCenter, pode ser mais fácil criar os grupos de recursos enquanto estiver criando o plano de replicação. Para obter instruções sobre como criar grupos de recursos enquanto estiver criando um plano de replicação, consulte link:drplan-create.html["Crie um plano de replicação"].
...
. No menu suspenso, filtre por *vCenter* grupos de recursos.
+
...
.Passos
. Em Disaster Recovery, selecione **Grupos de recursos** na barra lateral.
. No menu suspenso, filtre por sites *Kubernetes*.
+
image::site-type-toggle.png[Captura de tela da opção de alternância do tipo de site]
...
+
image:kubernetes-resource-group.png["Adicionar um grupo de recursos do Kubernetes"]
