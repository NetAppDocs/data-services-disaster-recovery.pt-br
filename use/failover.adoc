...
Antes de realizar um failover em produção, você pode executar um teste para verificar se está tudo pronto. As cargas de trabalho permanecem online durante o processo de teste.
...
+
Em seguida, digite “Test failover” e selecione *Test fail over*.
...
IMPORTANT: Se o plano de replicação incluir um datastore vazio, você deve primeiro remover o datastore vazio do plano ou usar a opção **Forçar failover**, que ignora os datastores vazios.
...
. Selecione *Forçar failover* se desejar que o failover ocorra mesmo se for detectado um erro que normalmente impediria o failover de ocorrer.
