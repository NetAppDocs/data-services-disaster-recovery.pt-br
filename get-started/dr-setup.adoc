...
[role="lead"]
Para utilizar o NetApp Disaster Recovery, primeiro prepare seu ambiente de recuperação antes de uma interrupção, para que a replicação e o failover possam ser configurados e testados com o mínimo de atraso.
...
* Recuperação de desastres em nuvem híbrida que replica um datacenter VMware local com ONTAP para uma infraestrutura de recuperação de desastres da AWS baseada em VMware Cloud on AWS ou Elastic VMware Service (Exchange Virtual Server) e Amazon FSx for NetApp ONTAP.
* Recuperação de desastres em nuvem privada que replica um ambiente VMware com ONTAP vCenter para outro ambiente VMware com ONTAP vCenter.
* Recuperação de desastres na nuvem que replica uma infraestrutura de recuperação de desastres da AWS baseada em VMware Cloud on AWS ou Exchange Virtual Server para outra infraestrutura de recuperação de desastres baseada na AWS usando FSx for NetApp ONTAP.
* Recuperação de desastres em nuvem híbrida que replica um datacenter VMware local com ONTAP para uma infraestrutura de recuperação de desastres do Google baseada no Google Cloud VMware Engine e no Google Cloud NetApp Volumes.
...
Este método consiste em uma infraestrutura de produção local vCenter usando datastores hospedados em volumes FlexVol do ONTAP por meio do protocolo NFS. O site de recuperação de desastres consiste em uma ou mais instâncias do VMware Cloud SDDC usando datastores hospedados em volumes FlexVol fornecidos por uma ou mais instâncias do FSx for ONTAP por meio do protocolo NFS.

Os sites de produção e recuperação de desastres são conectados por uma conexão segura compatível com a AWS. Os tipos de conexão comuns são uma VPN segura (privada ou fornecida pela AWS), AWS Direct Connect ou outros métodos de interconexão aprovados.
...
* *AWS Site-to-Site VPN*: Essa conexão de rede privada virtual pode fornecer conectividade segura por meio de um provedor de serviços de internet público. A VPN criptografa e descriptografa todos os dados que trafegam de e para a nuvem da AWS. As VPNs podem ser baseadas em software ou baseadas em hardware. Para aplicações corporativas, o provedor de serviços de internet (ISP) deve oferecer garantias de qualidade de serviço para garantir que haja largura de banda e latência adequadas para a replicação de recuperação de desastres.


*MELHOR PRÁTICA*: Para a configuração do site de recuperação de desastres na AWS, a NetApp recomenda usar o AWS Direct Connect. Essa solução oferece o mais alto desempenho e segurança para aplicações corporativas. Se não estiver disponível, deve-se usar uma conexão pública de ISP de alta performance juntamente com uma VPN. Certifique-se de que o ISP ofereça níveis de serviço de QoS comerciais para garantir desempenho de rede adequado.
...
*MELHOR PRÁTICA*: Para soluções de recuperação de desastres que utilizam o VMware Cloud e uma única VPC do Amazon FSx for NetApp ONTAP, a NetApp recomenda usar o peering de VPC. Se várias VPCs do Amazon FSx for NetApp ONTAP forem implantadas, recomendamos usar um AWS Transit Gateway para reduzir a sobrecarga no gerenciamento de múltiplas conexões de peering de VPC.
...
O NetApp Disaster Recovery suporta replicação site a site usando Ethernet e TCP/IP. Certifique-se de que haja largura de banda suficiente disponível para suportar as taxas de alteração de dados nas VMs do site de produção, para que todas as alterações possam ser replicadas para o site de NetApp Disaster Recovery dentro do prazo do Recovery Point Objective (RPO).
