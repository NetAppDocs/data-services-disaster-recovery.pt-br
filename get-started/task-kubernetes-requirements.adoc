...
[role="lead"]
Antes de configurar o NetApp Disaster Recovery para Kubernetes, prepare cada cluster Kubernetes para garantir sua prontidão para replicação e failover. A proteção normalmente se aplica a um cluster de origem e a um cluster de destino; conclua esses procedimentos em cada cluster do par.
...
* Certifique-se de estar usando uma versão compatível do Kubernetes. A Recuperação de Desastres oferece suporte a link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-requirements.html["qualquer versão do Kubernetes atualmente suportada pelo Trident Protect"^].
* Configure o link:../reference/trident-permissions.html["permissões para o Trident Protect"] apropriado.
...
* **LIF de dados (NFS)**: usado para tráfego NFS em volumes.
* **Nome da SVM**: a máquina virtual de armazenamento que hospeda os volumes.
* **Credenciais**: a conta que o Trident utiliza, geralmente  `admin` ou uma conta com escopo SVM.
...
Aplique um link:https://docs.netapp.com/us-en/trident/trident-use/backend-kubectl.html#step-2-create-the-tridentbackendconfig-cr[`TridentBackendConfig` que aponte para sua SVM^]. Por exemplo:
...
Instale o link:https://docs.netapp.com/us-en/trident/trident-use/vol-group-snapshots.html#deploy-a-volume-snapshot-controller["CRD de snapshot e controlador de snapshot"^]. Essas instalações são necessárias para snapshots de volume no Trident Protect.
...
== NetApp Console funções

Após realizar suas configurações do Trident Protect e do Kubernetes, link:https://docs.netapp.com/us-en/console-setup-admin/reference-iam-disaster-rec-roles.html["Certifique-se de ter as funções corretas do NetApp Console para o NetApp Disaster Recovery para executar a descoberta, o failover e outras tarefas."^]
