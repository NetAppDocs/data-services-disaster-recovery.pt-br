...
summary: A recuperação de desastres na nuvem é uma maneira resiliente e econômica de proteger cargas de trabalho contra interrupções no local e eventos de corrupção de dados. Com NetApp Disaster Recovery, você pode replicar suas cargas de trabalho VMware locais que executam armazenamento ONTAP para um data center definido por software em uma nuvem pública usando NetApp storage de nuvem ou para outro ambiente VMware local com armazenamento ONTAP como um site de recuperação de desastres. 
---
...
[role="lead"]
A recuperação de desastres na nuvem é uma maneira resiliente e econômica de proteger cargas de trabalho contra interrupções no site e eventos de corrupção de dados. NetApp Disaster Recovery é um serviço de recuperação de desastres baseado em nuvem que automatiza os fluxos de trabalho de recuperação de desastres. Com o NetApp Disaster Recovery, você pode proteger seus clusters Kubernetes locais, cargas de trabalho baseadas em NFS e datastores do sistema de arquivos de máquina virtual (VMFS) do VMware vSphere para iSCSI e FC executando armazenamento NetApp em uma das seguintes opções:

* Amazon Elastic VMware Service (Exchange Virtual Server) com Amazon FSx for NetApp ONTAP. Para obter detalhes, consulte link:../reference/evs-deploy-guide-introduction.html["Introdução ao NetApp Disaster Recovery com o Amazon Elastic VMware Service e Amazon FSx for NetApp ONTAP"].
* VMware Cloud (VMC) na AWS com Amazon FSx for NetApp ONTAP
* Solução Azure VMware com NetApp Cloud Volumes ONTAP (iSCSI) (prévia privada)
...
image:../use/evs-console-architecture.png["Diagrama mostrando os componentes da NetApp Disaster Recovery"]
...
== Benefícios de usar NetApp Disaster Recovery
...
* Realize um teste de failover. Ao fazer isso, o serviço cria máquinas virtuais temporárias. O Disaster Recovery cria um novo FlexClone volume a partir do snapshot selecionado, e um datastore temporário, com backup nesse volume FlexClone, é mapeado para os hosts ESXi. Esse processo não consome capacidade física adicional no armazenamento ONTAP local ou no FSx for NetApp ONTAP storage na AWS. O volume de origem original não é modificado e os trabalhos de replicação podem continuar mesmo durante a recuperação de desastres.
...
image:../use/evs-use-cases.png["Diagrama mostrando casos de uso para NetApp Disaster Recovery"]


NOTE: Você deve configurar o servidor vSphere fora do NetApp Disaster Recovery no vSphere Server.
...
* Adquira uma assinatura de pagamento conforme o uso (PAYGO) no Amazon Web Services (AWS) Marketplace, Azure Marketplace ou Google Cloud Marketplace. Essa licença permite que você adquira uma licença de capacidade protegida fixa sem qualquer compromisso de longo prazo.
...
O Disaster Recovery é licenciado com base na quantidade de dados hospedados em volumes ONTAP protegidos. O serviço determina quais volumes devem ser considerados para fins de licenciamento mapeando as VMs protegidas aos seus vCenter datastores. Cada datastore é hospedado em um volume ONTAP ou LUN. A capacidade utilizada relatada pelo ONTAP para esse volume ou LUN é usada para determinações de licenciamento.
...
NOTE: NetApp Disaster Recovery cobra com base na capacidade utilizada dos datastores no site de origem quando há pelo menos uma VM ou recurso Kubernetes com um plano de replicação. A capacidade de um datastore em failover não está incluída na franquia de capacidade. Para um BYOL, se os dados excederem a capacidade permitida, as operações no serviço serão limitadas até que você obtenha uma licença de capacidade adicional ou atualize a licença no NetApp Console.
...
Você pode experimentar NetApp Disaster Recovery com um período de teste gratuito de 30 dias.

Para continuar após o período de avaliação de 30 dias, você precisa obter uma assinatura Pay-as-you-go (PAYGO) do seu provedor de nuvem ou comprar uma licença BYOL da NetApp.
...
image:dr-architecture-diagram-70-2.png["Diagrama que mostra a arquitetura da infraestrutura do serviço NetApp Disaster Recovery"]
...
A recuperação de desastres utiliza os seguintes componentes:
...
* VMware Cloud (VMC) na AWS com Amazon FSx for NetApp ONTAP
* Outro ambiente VMware local baseado em NFS com armazenamento ONTAP ou um ambiente VMFS FC/iSCSI local
