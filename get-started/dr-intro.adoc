...
[role="lead"]
A recuperação de desastres na nuvem é uma maneira resiliente e econômica de proteger cargas de trabalho contra interrupções no site e eventos de corrupção de dados. Com NetApp Disaster Recovery, você pode replicar suas cargas de trabalho de máquina virtual VMware ou datastore locais que executam armazenamento ONTAP para um data center definido por software VMware em uma nuvem pública usando NetApp storage de nuvem ou para outro ambiente VMware local com armazenamento ONTAP como um site de recuperação de desastres. Você também pode usar Disaster Recovery para migrar cargas de trabalho de máquina virtual de um site para outro.

NetApp Disaster Recovery é um serviço de recuperação de desastres baseado em nuvem que automatiza os fluxos de trabalho de recuperação de desastres. Com o NetApp Disaster Recovery, você pode proteger seus clusters Kubernetes locais, cargas de trabalho baseadas em NFS e datastores do sistema de arquivos de máquina virtual (VMFS) do VMware vSphere para iSCSI e FC executando armazenamento NetApp em uma das seguintes opções:
...
A Recuperação de Desastres utiliza os seguintes componentes para fornecer cargas de trabalho de disaster Recovery:
