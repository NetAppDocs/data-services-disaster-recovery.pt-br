...
[role="lead"]
A conta do vCenter para o NetApp Disaster Recovery deve ter um conjunto mínimo de privilégios para registrar e cancelar o registro de datastores, iniciar e parar máquinas virtuais (VMs) e reconfigurar VMs.

A tabela a seguir lista os privilégios necessários para que o NetApp Disaster Recovery funcione com um vCenter cluster.
...
.2+| Host | Host.Config.AdvancedConfig | Configurações avançadas | Permite definir opções avançadas de configuração do host. 


| Host.Config.Storage | Configuração da partição de armazenamento | Permite a verificação de novos dispositivos de armazenamento e o gerenciamento de iSCSI. 


| Recurso | Recurso.AtribuirVMAoPool | Pools de recursos | Permite a atribuição de máquinas virtuais a um pool de recursos 
...
.12+| Configuração de máquina virtual | VirtualMachine.Config.AdvancedConfig | Configuração avançada | Permite adicionar ou modificar parâmetros avançados no arquivo de configuração da VM. 


| VirtualMachine.Config.AddExistingDisk | Adicionar disco existente | Permite adicionar um disco virtual existente a uma máquina virtual. 


| VirtualMachine.Config.AddNewDisk | Adicionar novo disco | Permite criar um novo disco virtual para adicionar a uma máquina virtual. 


| VirtualMachine.Config.RemoveDisk | Remover disco | Permite a remoção de um dispositivo de disco virtual. 
