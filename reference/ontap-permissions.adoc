...
[role="lead"]
Configure as permissões ONTAP necessárias para que o NetApp Disaster Recovery funcione corretamente.
...
O NetApp Disaster Recovery requer as seguintes permissões do ONTAP:
...
Para adicionar permissões no ONTAP, crie uma função com os privilégios necessários e atribua-a a um usuário. Você pode concluir essa tarefa no ONTAP com o System Manager ou a ONTAP CLI.
