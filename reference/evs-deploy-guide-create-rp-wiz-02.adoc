...
* *Selecionar grupos de recursos criados anteriormente*: Você pode criar grupos de recursos personalizados com antecedência usando a opção Grupo de recursos no menu NetApp Disaster Recovery .  Isso não é um requisito, pois você pode usar os outros dois métodos para criar um grupo de recursos como parte do processo de plano de replicação. Para obter detalhes, consulte link:../use/drplan-create.html["Crie um plano de replicação"] .
